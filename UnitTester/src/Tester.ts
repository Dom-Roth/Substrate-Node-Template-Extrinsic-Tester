import {ApiPromise, Keyring} from "@polkadot/api";
import fc from "fast-check";


export class Tester {
    /*
    Index:

        constructor()
        public printChainInformation()
        private handleCallback()
        private handleFailure()
        public testSuccessfulDoSomething()
        public testErrorDoSomething()
        public testSuccessfulCauseError()
        public testErrorCauseError()
        public fastCheckTest_DoSomething()
    */

    api;
    keyring;
    wallet_alice;
    wallet_bob;
    wallet_charlie;

    constructor(api:typeof ApiPromise) {
        this.api = api;
        this.keyring = new Keyring({ type: 'sr25519'});
        this.wallet_alice = this.keyring.addFromUri('//Alice');
        this.wallet_bob = this.keyring.addFromUri('//Bob');
        this.wallet_charlie = this.keyring.addFromUri('//Charlie');
    }

    public async printChainInformation() {
        const [chain, nodeName, nodeVersion] = await Promise.all([
            this.api.rpc.system.chain(),
            this.api.rpc.system.name(),
            this.api.rpc.system.version()
        ]);
        console.log(`\nYou are connected to chain ${chain} using ${nodeName} v${nodeVersion}`);
    }

    private handleCallback(testNum:number, result, callback, testDescription: string, intendedFail:boolean=false) {
        result.events.forEach(({event}) => {
            let success;
            if (this.api.events.system.ExtrinsicSuccess.is(event)) {
                success = intendedFail == false;
                callback(testNum, success, testDescription);
            } else if (this.api.events.system.ExtrinsicFailed.is(event)) {
                success = intendedFail == true;
                callback(testNum, success, testDescription);
            }
        });
    }

    private static handleFailure(testNum: number, callback, testDescription: string, intendedFail:boolean=false) {
        callback(testNum, intendedFail, testDescription);
    }

    public async testSuccessfulDoSomething(callback) {
        // Tests doSomething with valid data. Arg: 5
        try {
            let testDescription = "Tests doSomething => Valid data, arg: 5";
            await this.api.tx.templateModule.doSomething(5).signAndSend(this.wallet_alice, (result) => {
                this.handleCallback(1, result, callback, testDescription);
            });
        } catch (exc) {
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Valid data, arg: 5";
            Tester.handleFailure(1, callback, testDescription);
        }

        // Tests doSomething with valid data, before considering the bonus task. Arg: 5000
        try {
            let testDescription = "Tests doSomething => Valid data before bonus task, arg: 5000";
            await this.api.tx.templateModule.doSomething(5000).signAndSend(this.wallet_alice, {nonce:-1}, (result) => {
                this.handleCallback(2, result, callback, testDescription);
            });
        } catch (exc) {
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Valid data before bonus task, arg: 5000";
            Tester.handleFailure(2, callback, testDescription);
        }
    }

    public async testErrorDoSomething(callback) {
        // Tests expected errors. Most of them won't even reach the function on the blockchain since, they're blocked on the way.

        // Tests doSomething with invalid data. Sends a string as arg.
        try {
            let testDescription = "Tests doSomething => Invalid data: String arg: 'ABCDEF'";
            await this.api.tx.templateModule.doSomething("ABCDEF").signAndSend(this.wallet_bob, {nonce:-1}, (result) => {
                this.handleCallback(3, result, callback, testDescription, true);
            });
        } catch (exc) {
            // Expected to fail.
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Invalid data: String arg: 'ABCDEF'";
            Tester.handleFailure(3, callback, testDescription, true);
        }

        // Tests doSomething with invalid data. Sends long buffer overflow alike string.
        try {
            let testDescription = "Tests doSomething => Invalid data: Overflow String.";
            await this.api.tx.templateModule.doSomething("").signAndSend(this.wallet_alice, {nonce: -1}, (result) => {
                this.handleCallback(4, result, callback, testDescription, true);
            });
        } catch (exc) {
            // Expected to fail.
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Invalid data: Overflow String.";
            Tester.handleFailure(4, callback, testDescription, true);
        }

        // Tests doSomething with invalid data. Arg: -2
        try {
            let testDescription = "Tests doSomething => Invalid data, arg: -2.";
            await this.api.tx.templateModule.doSomething(-2).signAndSend(this.wallet_bob, {nonce:-1}, (result) => {
                this.handleCallback(5, result, callback, testDescription, true);
            });
        } catch (exc) {
            // Expected to fail.
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Invalid data, arg: -2.";
            Tester.handleFailure(5, callback, testDescription, true);
        }

        // Tests doSomething with invalid data. Sends using an account without funds.
        try {
            let testDescription = "Tests doSomething => Invalid data, overflow int arg: 4294967296.";
            await this.api.tx.templateModule.doSomething(4294967296).signAndSend(this.wallet_bob, {nonce:-1}, (result) => {
                this.handleCallback(6, result, callback, testDescription, true);
            });
        } catch (exc) {
            // Expected to fail.
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Invalid data, overflow int arg: 4294967296.";
            Tester.handleFailure(6, callback, testDescription, true);
        }

        // Tests doSomething with invalid data. Sends using an account without funds.
        try {
            let testDescription = "Tests doSomething => Invalid data, no fee money.";
            await this.api.tx.templateModule.doSomething(1).signAndSend(this.wallet_charlie, {nonce:-1}, (result) => {
                this.handleCallback(7, result, callback, testDescription, true);
            });
        } catch (exc) {
            // Expected to fail.
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Invalid data, no fee money.";
            Tester.handleFailure(7, callback, testDescription, true);
        }

        // Tests doSomething with invalid data. Sends using an account without funds.
        try {
            let testDescription = "Tests doSomething => Invalid data, not signed.";
            await this.api.tx.templateModule.doSomething(1).send((result) => {
                this.handleCallback(8, result, callback, testDescription, true);
            });
        } catch (exc) {
            // Expected to fail.
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests doSomething => Invalid data, not signed.";
            Tester.handleFailure(8, callback, testDescription, true);
        }
    }

    public async testSuccessfulCauseError(callback) {
        // Tests causeError with valid data. After running all other tests
        try {
            let testDescription = "Tests causeError => Valid data";
            await this.api.tx.templateModule.causeError().signAndSend(this.wallet_alice, (result) => {
                this.handleCallback(1, result, callback, testDescription);
            });
        } catch (exc) {
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests causeError => Valid data, arg: 5";
            Tester.handleFailure(1, callback, testDescription);
        }
    }

    public async testErrorCauseError(callback) {
        // Tests causeError while 'Something' is still unset.
        // Does only work if application only tests causeError and wasn't set before.
        // If this test fails, please reset the blockchain.
        try {
            let testDescription = "Tests causeError => Unset Storage";
            await this.api.tx.templateModule.causeError().signAndSend(this.wallet_bob, {nonce: -1}, (result) => {
                this.handleCallback(1, result, callback, testDescription, true);
            });
        } catch (exc) {
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests causeError => Unset Storage";
            Tester.handleFailure(1, callback, testDescription, true);
        }

        // Tests causeError by sending the request unsigned.
        try {
            let testDescription = "Tests causeError => Unsigned request";
            await this.api.tx.templateModule.causeError().send((result) => {
                this.handleCallback(3, result, callback, testDescription, true);
            });
        } catch (exc) {
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests causeError => Unsigned request";
            Tester.handleFailure(3, callback, testDescription, true);
        }

        // Tests causeError with invalid data. First uses doSomething with 4294967295 and then calls causeError to overflow.
        // Does fail if the assert of the bonus task is added. Because we can not set >Something< to a value close
        // to the maximum of an u32 Integer.
        try {
            let testDescription = "Tests causeError => Overflow Storage";
            await this.api.tx.templateModule.doSomething(4294967295).signAndSend(this.wallet_alice, {nonce:-1}).finally(() => {
                this.api.tx.templateModule.causeError().signAndSend(this.wallet_alice, {nonce: -1}, (result) => {
                    this.handleCallback(2, result, callback, testDescription, true);
                });
            });
        } catch (exc) {
            // ToDo: Add boolean switch for debug msgs.
            let testDescription = "Tests causeError => Overflow Storage";
            Tester.handleFailure(2, callback, testDescription, true);
        }
    }

    public async fastCheckTest_DoSomething() {
        await fc.assert(
            fc.asyncProperty(
                fc.integer({min: 1330, max: 1340}), async (i:number) => {
                    await this.api.tx.templateModule.doSomething(i).signAndSend(this.wallet_bob, {nonce: -1}, (result) => {
                        result.events.forEach(({event}) => {
                            // Unfortunately doesn't work since the script never receives an answer due to the panic.
                            if (this.api.events.system.ExtrinsicSuccess.is(event)) {
                                return true;
                            }
                        });
                        return result.status.isReady == true || result.status.isInBlock == true;
                    });
                }
            )
        );
    }
}